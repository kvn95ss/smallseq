...
class GenomeFetch:
    ...
    def __init__(self, species=None,assembly=None, genomedir=None):
        ...
        if genomedir is not None:
            ...
            if species is not None:
                ...
                species_from_assembly = dict((a,s) for s,a_arr in list(self.fasta_directories.items()) for a in a_arr)
            ...
            if assembly is None:
                ...
                if self.assembly not in self.fasta_directories[self.species]:
    ...
    def get_seq_from_to(self, chromosome,coordFrom, coordTo,strand=None, fLeaveOpen=False, Fa=True):
        ...
        rawSeq = self.fileSeq.read(int(abs( coordFrom - coordTo) + 1 + retbeforeEnd)).replace("\n","").upper()
